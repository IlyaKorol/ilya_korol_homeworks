# Задача 2. Есть Помидор со следующими характеристиками:
# 1)Индекс
...
# 2)Создайте метод __init__(), внутри которого будут определены два атрибута: 1) name - передается параметром, является публичным и 2) _plant - принимает объект класса TomatoBush, является приватным
...
class Tomato:
    ...
    def grow(self):  # метод, переводящий томат на следующую стадию
        if self._state < 3:
            self._state += 1

    def is_ripe(self):  # Проверка зрелости
...
class Gardener:
    ...
    def harvest(self):
        if self._plant.all_are_ripe():
            print('Все плоды созрели, садовник убирает урожай')
        else:
            print('Еще не все плоды созрели')

    @staticmethod
    def knowledge_base():
        print('Справка по садоводству: для сбора урожая все плоды должны достигнуть стадии "Красный"')


Gardener.knowledge_base()

tomat = TomatoBush(5)
gard = Gardener('Василий', tomat)
print(f'Начало работы садовника:')
gard.work()
gard.harvest()
print('Продолжение работы садовника:')
gard.work()
gard.harvest()
print('Завершение работы садовника:')
gard.work()
gard.harvest()
